...
const DownloadReport = () => {
  ...
  const [paymentDialogOpen, setPaymentDialogOpen] = useState(false);  const [ratingDialogOpen, setRatingDialogOpen] = useState(false);  const [filters, setFilters] = useState({
    search: '',
    dateFrom: '',
    dateTo: ''
  });
  ...
  const FIXED_FEE = 0.30; // Stripe fixed fee in USD

  // Calculate total payment amount
  const calculatePaymentAmount = (estimatedAmount) => {
    const platformFee = (estimatedAmount * PLATFORM_FEE_PERCENTAGE) / 100;
    const stripeFee = ((estimatedAmount + platformFee) * PAYMENT_PROCESSING_FEE) / 100 + FIXED_FEE;
      ...
      <div className="mb-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">Access your completed job reports and attachments</p>
          </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center h-64">
            ...
            <h3 className="text-lg font-semibold text-gray-600 mb-2">No Completed Jobs Found</h3>
            <p className="text-gray-500 text-center">
              You don't have any completed jobs yet or no jobs match your filters.
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredJobs.map((job) => {
            const paymentDetails = calculatePaymentAmount(job.estimatedTotal);
            const isPaid = job.paymentStatus === 'paid';
            const hasRating = job.clientRating && job.clientRating.rating > 0;
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>{formatDate(job.actualCompletionDate)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Paperclip className="h-4 w-4 text-gray-500" />
                      <span className="text-gray-600">Attachments:</span>
                      <span>{job.attachments?.length || 0} file(s)</span>
                    </div>
                  </div>
                  ...
                  {hasRating && (
                    <div className="bg-yellow-50 p-3 rounded-lg">
                      ...
                      <StarRating value={job.clientRating.rating} readonly />
                      {job.clientRating.review && (
                        <p className="text-sm text-gray-600 mt-2">{job.clientRating.review}</p>
                      )}
                    </div>
                  )}
                  ...
                  <div className="flex flex-col gap-2 pt-2">
                    {!isPaid ? (
                      ...
                      >
                        ...
                        Pay & Download ({formatCurrency(paymentDetails.totalAmount)})
                      </Button>
                    ) : (
                        ...
                        <Dialog>
                          <DialogTrigger asChild>
                            ...
                            >
                              ...
                              View Attachments ({job.attachments?.length || 0})
                            </Button>
                          </DialogTrigger>
                          <DialogContent className="max-w-2xl">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Download attachments for: {job.title}
                              </DialogDescription>
                            </DialogHeader>
                            <AttachmentsModal job={job} onDownload={downloadAttachment} />
                          </DialogContent>
                        </Dialog>
      ...
      <Dialog open={paymentDialogOpen} onOpenChange={setPaymentDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Pay to access reports and attachments for: {selectedJob?.title}
            </DialogDescription>
          </DialogHeader>
            {selectedJob && (
            <StripePaymentForm
              job={selectedJob}
              paymentDetails={calculatePaymentAmount(selectedJob.estimatedTotal)}
              onSuccess={() => {

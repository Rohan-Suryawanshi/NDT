...
const InspectorWithdraw = () => {
  ...
  const fetchWithdrawHistory = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`${BACKEND_URL}/api/v1/payments/withdraw-history`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const filteredPayments = paymentHistory.filter(payment => {
    const matchesSearch = !filters.search || 
      payment.jobTitle?.toLowerCase().includes(filters.search.toLowerCase()) ||
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-2xl font-bold">{formatCurrency(balance.totalWithdrawn)}</div>
                  </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-green-600">{formatCurrency(balance.totalEarnings)}</span>
          </div>
          <div className="flex justify-between">
            <span>Available Balance:</span>
            <span className="font-semibold text-blue-600">{formatCurrency(balance.availableBalance)}</span>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold">{formatCurrency(averageWithdrawal)}</span>
          </div>
          <div className="flex justify-between">
            <span>Total Withdrawn:</span>
            <span className="font-semibold text-purple-600">{formatCurrency(balance.totalWithdrawn)}</span>
          </div>
          <div className="flex justify-between">
            <span>Pending Balance:</span>
            <span className="font-semibold text-yellow-600">{formatCurrency(balance.pendingBalance)}</span>
          </div>
        </CardContent>
      </Card>
    </div>

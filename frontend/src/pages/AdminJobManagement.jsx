...
const AdminJobManagement = () => {
  ...
  const handleStatusUpdate = async (jobId, newStatus, reason = "") => {
    try {
      const api = createAxiosInstance();
      const response = await api.patch(`/api/v1/job-requests/${jobId}/status`, {
        status: newStatus,
        reason,
      });
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center py-6">
              <div className="flex items-center space-x-4">
                ...
                <span className="bg-blue-100 text-[#004aad] text-sm font-medium px-3 py-1 rounded-full">
                  {pagination.totalJobs} Total Jobs
                </span>
              </div>
              <div className="flex items-center space-x-3">
                <Button onClick={fetchJobRequests} variant="outline" size="sm">
                                ...
                                <TableCell>
                                  <div className="space-y-1">
                                    ...
                                    {job.finalQuotedAmount && (
                                      <p className="text-sm text-gray-600">
                                        Final: {formatCurrency(job.finalQuotedAmount)}
                                      </p>
                                    )}
                                  </div>
                                </TableCell>
                                    ...
                                    {job.quotationHistory?.length > 0 && (
                                      <div className="flex items-center space-x-1">
                                        ...
                                        <span className="text-sm text-blue-600">
                                          {job.quotationHistory.length} Quote
                                    ...
                                    {job.internalNotes?.length > 0 && (
                                      <div className="flex items-center space-x-1">
                                        ...
                                        <span className="text-sm text-purple-600">
                                          {job.internalNotes.length} Note
                                          {job.internalNotes.length > 1 ? "s" : ""}
                                        </span>
                                      </div>
                                    )}
                                  </div>
                                  ...
                                  <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                      ...
                                      {(job.status === "completed" || job.status === "closed") && (
                                        <DropdownMenuItem
                                          onClick={() => handleGenerateReport(job._id)}
                                        >
                                    ...
                                    <div className="p-4 bg-gray-50 rounded-lg space-y-4">
                                      ...
                                      <div className="flex space-x-2 pt-2 border-t">
                                        ...
                                        <Select
                                          value={job.status}
                                          onValueChange={(value) =>
                                            handleStatusUpdate(job._id, value)
                                          }
                                        >
          ...
          <Dialog open={showJobModal} onOpenChange={setShowJobModal}>
            <DialogContent className="max-w-6xl max-h-screen overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Complete job information including quotations, notes, and progress tracking
                </DialogDescription>
              </DialogHeader>
                ...
                <Tabs defaultValue="overview" className="w-full">
                  <TabsList className="grid w-full grid-cols-5">
                    ...
                    <TabsTrigger value="quotations">
                      Quotations ({selectedJob.quotationHistory?.length || 0})
                    </TabsTrigger>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                              <Label>Region</Label>
                              <p className="font-medium">{selectedJob.region}</p>
                            </div>
                            <div>
                              <Label>Duration</Label>
                              <p className="font-medium">
                                {selectedJob.projectDuration} day
                                {selectedJob.projectDuration > 1 ? "s" : ""}
                              </p>
                            </div>
                            <div>
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-1">
                              {selectedJob.requiredServices?.map((service) => (
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          ...
                          {selectedJob.clientPhone && (
                            <div>
                              <Label>Phone</Label>
                              <p className="font-medium">{selectedJob.clientPhone}</p>
                            </div>
                          )}
                        </CardContent>
                      </Card>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-xl font-bold text-green-600">
                                {formatCurrency(selectedJob.estimatedTotal)}
                              </p>
                            </div>
                            {selectedJob.finalQuotedAmount && (
                              <div>
                                <Label>Final Quoted Amount</Label>
                                <p className="text-xl font-bold text-blue-600">
                                  {formatCurrency(selectedJob.finalQuotedAmount)}
                                </p>
                              </div>
                            )}
                          </div>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="bg-gray-50 p-3 rounded-lg space-y-2">
                                <div className="flex justify-between text-sm">
                                  <span>Base Cost:</span>
                                  <span>
                                    {formatCurrency(selectedJob.costDetails.totals?.baseCost)}
                                  </span>
                                </div>
                                <div className="flex justify-between text-sm">
                                  <span>Tax:</span>
                                  <span>
                                    {formatCurrency(selectedJob.costDetails.totals?.tax)}
                                  </span>
                                </div>
                                {selectedJob.costDetails.totals?.additional > 0 && (
                                  <div className="flex justify-between text-sm">
                                    ...
                                    <span>
                                      {formatCurrency(selectedJob.costDetails?.totals?.additional)}
                                    </span>
                                  </div>
                                )}
                        ...
                        <Card>
                          ...
                          <CardContent className="space-y-4">
                            <div>
                              ...
                              <p className="font-medium">{selectedJob.providerName}</p>
                            </div>
                            {selectedJob.assignedProviderId && (
                  ...
                  <TabsContent value="quotations" className="space-y-4">
                    {selectedJob.quotationHistory && selectedJob.quotationHistory.length > 0 ? (
                              ...
                              <CardContent className="p-6">
                                ...
                                {quotation.negotiations && quotation.negotiations.length > 0 && (
                                  <div className="mt-4 border-t pt-4">
                                    <Label className="mb-2 block">Negotiation History</Label>
                  ...
                  <TabsContent value="notes" className="space-y-4">
                    {selectedJob.internalNotes && selectedJob.internalNotes.length > 0 ? (
                      <div className="space-y-4">
                      ...
                      {selectedJob.preferredStartDate && (
                        <div className="flex items-center space-x-4 p-4 bg-yellow-50 rounded-lg">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {formatDate(selectedJob.preferredStartDate)}
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {selectedJob.actualStartDate && (
                        <div className="flex items-center space-x-4 p-4 bg-green-50 rounded-lg">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {formatDate(selectedJob.actualStartDate)}
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {selectedJob.expectedCompletionDate && (
                        <div className="flex items-center space-x-4 p-4 bg-purple-50 rounded-lg">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {formatDate(selectedJob.expectedCompletionDate)}
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {selectedJob.actualCompletionDate && (
                        <div className="flex items-center space-x-4 p-4 bg-green-50 rounded-lg">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {formatDate(selectedJob.actualCompletionDate)}
                            </p>
                          </div>
                        </div>
                      )}
              ...
              <DialogFooter className="flex justify-between">
                <div className="flex space-x-2">
                  {(selectedJob.status === "completed" || selectedJob.status === "closed") && (
                    <Button
                      variant="outline"
                      onClick={() => handleGenerateReport(selectedJob._id)}
                    >
                      <Download className="h-4 w-4 mr-2" />
                      Download Report
                    </Button>

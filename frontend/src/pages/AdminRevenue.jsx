...
const AdminRevenue = () => {
  ...
  const fetchRevenueStats = useCallback(async () => {
    try {
      ...
      const response = await axiosInstance.get('/api/v1/payments/admin/stats');
      ...
      if (response.data.success) {
        ...
        setRevenueData({
          ...
          monthlyTrends: data.monthlyTrends || { payments: [], withdrawals: [] },
  ...
  const fetchPaymentsHistory = useCallback(async () => {
    try {
      const axiosInstance = createAxiosInstance();
      const response = await axiosInstance.get('/api/v1/payments/history?limit=10');
  ...
  const fetchWithdrawalsHistory = useCallback(async () => {
    try {
      const axiosInstance = createAxiosInstance();
      const response = await axiosInstance.get('/api/v1/payments/admin/withdrawals?limit=10');
  ...
  useEffect(() => {
    ...
    initializeData();
  }, [fetchRevenueStats, fetchPaymentsHistory, fetchWithdrawalsHistory, fetchPlatformSettings]);
  ...
  const calculateMetrics = () => {
    const platformRevenue = revenueData.totalPayments - revenueData.totalWithdrawals;
    const conversionRate = revenueData.totalUsers > 0 
      ? (revenueData.activeUsers / revenueData.totalUsers) * 100 
      : 0;
    
    // Calculate processing fees using real platform settings
    const processingFees = revenueData.totalPayments * (platformSettings.processingFeePercentage / 100);
    ...
    return {
      ...
      conversionRate,
      processingFees,
      avgPaymentValue: paymentsHistory.length > 0 ? revenueData.totalPayments / paymentsHistory.length : 0,
      pendingWithdrawalRate: revenueData.totalWithdrawalsCount > 0 
        ? (revenueData.pendingWithdrawals / revenueData.totalWithdrawalsCount) * 100 
        : 0,
  ...
  const withdrawalStatusData = revenueData.withdrawalStatusCounts.map(item => ({
    name: item._id,
    value: item.count,
    color: {
      'pending': '#f59e0b',
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Average Payment Value</CardTitle>
            </CardHeader>
            <CardContent>
            ...
            <CardContent>
              ...
              <div className="flex items-center mt-2">
                <span className="text-sm text-gray-600">
                  {formatPercentage(metrics.pendingWithdrawalRate)} of total requests
                </span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="flex items-center mt-2">
                <span className="text-sm text-gray-600">
                  {formatPercentage(platformSettings.processingFeePercentage)} + ${platformSettings.fixedProcessingFee}
                </span>
              </div>
            </CardContent>
          </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {withdrawalsHistory.slice(0, 5).map((withdrawal, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div>
                          <p className="font-medium">{formatCurrency(withdrawal.amount)}</p>
                          <p className="text-sm text-gray-600">{withdrawal.withdrawalMethod}</p>
                        </div>
                        <Badge 
                          variant={
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                      ...
                      <span className="text-xl font-bold text-blue-600">
                        {formatPercentage(platformSettings.platformFeePercentage)}
                      </span>
                    </div>
                    
                    <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                      <div>
                        <p className="font-medium text-green-900">Provider Commission</p>
                        <p className="text-sm text-green-700">Provider keeps</p>
                      </div>
                      <span className="text-xl font-bold text-green-600">
                        {formatPercentage(platformSettings.providerCommissionPercentage)}
                      </span>
                    </div>
                    
                    <div className="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
                      <div>
                        <p className="font-medium text-purple-900">Inspector Commission</p>
                        <p className="text-sm text-purple-700">Inspector keeps</p>
                      </div>
                      <span className="text-xl font-bold text-purple-600">
                        {formatPercentage(platformSettings.inspectorCommissionPercentage)}
                      </span>
                    </div>
                    
                    <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg">
                      <div>
                        <p className="font-medium text-red-900">Processing Fee</p>
                        <p className="text-sm text-red-700">Stripe processing</p>
                      </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">Minimum amount allowed</p>
                      </div>
                      <span className="text-xl font-bold">
                        {formatCurrency(platformSettings.minimumWithdrawalAmount)}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className="text-xl font-bold">
                        {platformSettings.withdrawalProcessingDays} days
                      </span>
                    </div>
                    
                    <div className="mt-4">
                      <h4 className="font-medium mb-2">Withdrawal Method Fees</h4>
                      <div className="space-y-2">
                        {Object.entries(platformSettings.withdrawalFees || {}).map(([method, fees]) => (
                          <div key={method} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                            ...
                            <span className="text-sm">
                              {fees.percentage > 0 ? `${formatPercentage(fees.percentage)}` : ''}
                              {fees.percentage > 0 && fees.fixed > 0 ? ' + ' : ''}
                              {fees.fixed > 0 ? `$${fees.fixed}` : ''}
                              {fees.percentage === 0 && fees.fixed === 0 ? 'Free' : ''}
                            </span>
                          </div>
                        ))}
                      </div>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="text-center p-4 bg-blue-50 rounded-lg">
                    <h4 className="font-bold text-blue-900 mb-2">Gross Revenue</h4>
                    <p className="text-2xl font-bold text-blue-600">
                      {formatCurrency(revenueData.totalPayments)}
                    </p>
                    <p className="text-sm text-blue-700 mt-1">Total payments received</p>
                  </div>
                  
                  <div className="text-center p-4 bg-red-50 rounded-lg">
                    <h4 className="font-bold text-red-900 mb-2">Total Costs</h4>
                    <p className="text-2xl font-bold text-red-600">
                      {formatCurrency(revenueData.totalWithdrawals + metrics.processingFees)}
                    </p>
                    <p className="text-sm text-red-700 mt-1">Payouts + processing fees</p>
                  </div>
                  
                  <div className="text-center p-4 bg-green-50 rounded-lg">
                    <h4 className="font-bold text-green-900 mb-2">Net Platform Revenue</h4>
                    <p className="text-2xl font-bold text-green-600">
                      {formatCurrency(metrics.netPlatformRevenue)}
                    </p>
                    <p className="text-sm text-green-700 mt-1">
                      {formatPercentage((metrics.netPlatformRevenue / revenueData.totalPayments) * 100)} margin
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <Table>
                    ...
                    <TableBody>
                      {paymentsHistory.slice(0, 5).map((payment, index) => (
                        <TableRow key={index}>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm">{formatCurrency(revenueData.totalPayments)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">Provider Payouts</span>
                      <span className="text-sm text-red-600">-{formatCurrency(revenueData.totalWithdrawals)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">
                        Processing Fees ({formatPercentage(platformSettings.processingFeePercentage)} + ${platformSettings.fixedProcessingFee})
                      </span>
                      <span className="text-sm text-red-600">-{formatCurrency(metrics.processingFees)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">
                        Platform Fee ({formatPercentage(platformSettings.platformFeePercentage)})
                      </span>
                      <span className="text-sm text-green-600">
                        +{formatCurrency(revenueData.totalPayments * (platformSettings.platformFeePercentage / 100))}
                      </span>
                    </div>
                    <hr />
                    <div className="flex justify-between items-center font-bold">
                      <span>Net Platform Revenue</span>
                      <span className="text-green-600">{formatCurrency(metrics.netPlatformRevenue)}</span>
                    </div>
                    <div className="text-xs text-gray-500 mt-2">
                      Profit Margin: {formatPercentage((metrics.netPlatformRevenue / revenueData.totalPayments) * 100)}
                    </div>
                  </div>
                </CardContent>
              </Card>

...
const ServiceProviderFeedback = () => {
   ...
   const fetchFeedbacks = useCallback(async () => {
      try {
         ...
         const jobsWithRatings = jobs.filter(
            (job) => job.clientRating && job.clientRating.rating
         );
         ...
         const averageRating =
            totalFeedbacks > 0
               ? jobsWithRatings.reduce(
                    (sum, job) => sum + job.clientRating.rating,
                    0
                 ) / totalFeedbacks
               : 0;
         const positiveRatings = jobsWithRatings.filter(
            (job) => job.clientRating.rating >= 4
         ).length;

         // Calculate recent ratings (last 30 days)
         const recentRatings = jobsWithRatings.filter(
            (job) =>
               new Date(job.clientRating.submittedAt) >
   ...
   const filteredFeedbacks = jobRequests
      ...
      .filter((job) => {
         ...
         const matchesRating =
            filters.rating === "all" ||
            feedback.rating.toString() === filters.rating;

         const matchesServiceType =
            filters.serviceType === "all" ||
            job.requiredServices?.some((service) =>
               service.name?.includes(filters.serviceType)
            );
               ...
               <>
                  ...
                  <Tabs defaultValue="feedback" className="space-y-6">
                     <TabsList className="grid w-full grid-cols-3">
                        <TabsTrigger value="feedback" className="data-[state=active]:bg-[#004aad] data-[state=active]:text-white data-[state=active]:shadow-sm">
                           Client Feedback
                        </TabsTrigger>
                        ...
                        <TabsTrigger value="analytics" className="data-[state=active]:bg-[#004aad] data-[state=active]:text-white data-[state=active]:shadow-sm">
                           ...
                           <CardHeader>
                              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                 <div>
                                    ...
                                    <CardDescription>
                                       Reviews and ratings from clients after
                                       service completion
                                    </CardDescription>
                                 </div>
                                 <div className="flex gap-2">
                     ...
                     <TabsContent value="jobs">
                        <Card>
                           <CardHeader>
                              ...
                              <CardDescription>
                                 All your completed service jobs and their
                                 feedback status
                              </CardDescription>
                           </CardHeader>
                           <CardContent>
      ...
      return (
         <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm mt-1">
               Client feedback will appear here after service completion
            </p>
         </div>
      );
   }
                  ...
                  <CardContent className="pt-4">
                     <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex flex-col items-end gap-2">
                           <Badge
                              className={ratingConfig[feedback.rating]?.color}
                           >
...
const ServiceFeedbackAnalytics = ({ feedbacks, stats }) => {
   ...
   const ratingDistribution = Array.from({ length: 5 }, (_, i) => {
      ...
      const count = feedbacks.filter(
         (job) => job.clientRating?.rating === rating
      ).length;
      const percentage =
         feedbacks.length > 0
               ...
               <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-blue-600">
                     {stats.totalFeedbacks > 0
                        ? (
                             (stats.positiveRatings / stats.totalFeedbacks) *
                             100
                          ).toFixed(1)
                        : 0}
                     %

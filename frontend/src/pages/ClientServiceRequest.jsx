...
const STATUS_CONFIG = {
  ...
  open: { label: 'Open', color: 'bg-blue-100 text-blue-800 border-blue-200', icon: Clock3 },
  quoted: { label: 'Quoted', color: 'bg-green-100 text-green-800 border-green-200', icon: Quote },
  negotiating: { label: 'Negotiating', color: 'bg-yellow-100 text-yellow-800 border-yellow-200', icon: Handshake },
  accepted: { label: 'Accepted', color: 'bg-purple-100 text-purple-800 border-purple-200', icon: CheckCheck },
  in_progress: { label: 'In Progress', color: 'bg-orange-100 text-orange-800 border-orange-200', icon: Activity },
  completed: { label: 'Completed', color: 'bg-emerald-100 text-emerald-800 border-emerald-200', icon: CheckCircle2 },
  delivered: { label: 'Delivered', color: 'bg-teal-100 text-teal-800 border-teal-200', icon: Send },
  closed: { label: 'Closed', color: 'bg-green-100 text-green-800 border-green-200', icon: CheckCircle2 },
  cancelled: { label: 'Cancelled', color: 'bg-red-100 text-red-800 border-red-200', icon: XCircle },
  rejected: { label: 'Rejected', color: 'bg-red-100 text-red-800 border-red-200', icon: XCircle },
  disputed: { label: 'Disputed', color: 'bg-red-100 text-red-800 border-red-200', icon: AlertCircle },
  on_hold: { label: 'On Hold', color: 'bg-yellow-100 text-yellow-800 border-yellow-200', icon: Clock },
};
...
const QUOTATION_STATUS = {
  ...
  negotiating: { label: 'Negotiating', color: 'bg-blue-100 text-blue-800', icon: MessageCircle },
};
...
const ClientServiceRequest = () => {
  ...
  const handleExportRequests = useCallback(async () => {
    try {
      ...
      const csvHeaders = ['Title', 'Status', 'Location', 'Region', 'Created Date', 'Estimated Total', 'Provider', 'Services'];
      const csvRows = requests.map(request => [
        ...
        request.requiredServices?.map(s => s.name || s.code).join('; ') || 'N/A'
      ]);
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const newStats = {
          ...
          open: requestData.filter(r => ['draft', 'open'].includes(r.status)).length,
          quoted: requestData.filter(r => ['quoted', 'negotiating'].includes(r.status)).length,
          inProgress: requestData.filter(r => ['accepted', 'in_progress'].includes(r.status)).length,
          completed: requestData.filter(r => ['completed', 'delivered', 'closed'].includes(r.status)).length,
          cancelled: requestData.filter(r => ['cancelled', 'rejected', 'disputed'].includes(r.status)).length,
        };
        setStats(newStats);
      } else {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch service requests';
  ...
  const retryFetch = useCallback(async (retryCount = 0) => {
    ...
    try {
      ...
      if (retryCount < maxRetries) {
        ...
        setError('Failed to load service requests after multiple attempts');
      }
    }
  }, [fetchRequests]);
  ...
  }, [showDetailsModal, showNegotiationModal, requests.length, handleExportRequests, fetchRequests]);
    ...
    const pollForUpdates = setInterval(async () => {
      try {
        ...
        if (response.data.success) {
          ...
          const currentQuotationCount = selectedRequest.quotationHistory?.length || 0;
          const newQuotationCount = updatedRequest.quotationHistory?.length || 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">      {/* Header */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">        {/* Error Display */}
        {error && (
        ...
        <div className="flex items-center justify-between mb-6">
          <Alert className="flex-1 border-blue-200 bg-blue-50 mr-4">
            <AlertDescription className="text-sm">
              <strong>Keyboard shortcuts:</strong> Ctrl+R (Refresh) • Ctrl+E (Export) • Ctrl+F (Search)
            </AlertDescription>
          </Alert>
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-6 mb-8">
          {Object.entries(stats).map(([key, value]) => {
            ...
            switch(key) {
              case 'total':
                config = { label: 'Total', color: 'bg-blue-100 text-blue-800', icon: FileText };
                displayLabel = 'Total';
                break;
              case 'open':
              ...
              case 'cancelled':
                ...
                break;
              default:
                config = { label: key, color: 'bg-gray-100 text-gray-800', icon: Activity };
                displayLabel = key;
            }
        ...
        <Card className="mb-6">
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">              {/* Search */}
              ...
              <Button 
                variant="outline" 
                onClick={() => setFilters({ search: '', status: 'all', dateFrom: '', dateTo: '', location: '' })}
              >
                Clear All
              </Button>
            </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {requests.map((request) => {
            ...
            const hasQuotations = request.quotationHistory && request.quotationHistory.length > 0;
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    <div className="flex items-center text-sm text-gray-600">
                      ...
                      <span>{request.projectDuration} day{request.projectDuration > 1 ? 's' : ''}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-4 p-3 bg-green-50 rounded-lg">
                    ...
                    <span className="text-lg font-bold text-green-600">
                      {formatCurrency(request.estimatedTotal, request.costDetails?.currency)}
                    </span>
                  </div>
                    ...
                    <div className="mb-4 p-3 bg-blue-50 rounded-lg">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-blue-900">
                          {request.quotationHistory.length} Quotation{request.quotationHistory.length > 1 ? 's' : ''}
                        </span>
                        <Quote className="h-4 w-4 text-blue-600" />
                      </div>
                      {request.quotationHistory.length > 0 && (                        <div className="text-xs text-blue-700 mt-1">
                          Latest: {formatCurrency(request.quotationHistory[request.quotationHistory.length - 1]?.quotedAmount)}
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {request.requiredServices?.slice(0, 3).map((service) => (
                      ...
                      {request.requiredServices?.length > 3 && (
                        <Badge variant="secondary" className="text-xs">
                          +{request.requiredServices.length - 3} more
                        </Badge>
                      )}
                    </div>
                  </div>                  {/* Actions */}
                  <div className="flex space-x-2 pt-4 border-t border-gray-100">
                    ...
                    {hasQuotations && (
                      ...
                      >
                        ...
                        Quotations ({request.quotationHistory.length})
                      </Button>
                    )}
                  </div>
                </CardContent>
        ...
        {pagination.totalPages > 1 && (
          <div className="flex items-center justify-between mt-8">
            <div className="text-sm text-gray-700">
              ...
              {Math.min(pagination.currentPage * pagination.pageSize, pagination.totalRequests)} of{' '}
            ...
            <Tabs defaultValue="overview" className="w-full">
              <TabsList className="grid w-full grid-cols-4">
                ...
                <TabsTrigger value="quotations">
                  Quotations ({selectedRequest.quotationHistory?.length || 0})
                </TabsTrigger>
                <TabsTrigger value="cost-breakdown">Cost Details</TabsTrigger>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-medium">{selectedRequest.projectDuration} days</p>
                        </div>
                        <div>
                          <Label>Inspectors</Label>
                          <p className="font-medium">{selectedRequest.numInspectors}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <TabsContent value="quotations" className="space-y-6">
                {selectedRequest.quotationHistory && selectedRequest.quotationHistory.length > 0 ? (
                  <div className="space-y-4">
                    {selectedRequest.quotationHistory.map((quotation, index) => {
                      ...
                      return (
                        <Card key={index} className="border-l-4 border-l-blue-500">
                          <CardContent className="p-6">
                            <div className="flex items-start justify-between mb-4">                              <div>
                                <div className="flex items-center space-x-2 mb-2">
                                  <h4 className="text-lg font-semibold text-green-600">
                                    {formatCurrency(quotation.quotedAmount, quotation.quotedCurrency)}
                                  </h4>
                                ...
                                <p className="text-sm text-gray-600">
                                  Valid until: {quotation.validUntil ? formatDate(quotation.validUntil) : 'No expiry'}
                                </p>
                                <p className="text-sm text-gray-600">
                            ...
                            </div>                            {quotation.quotationDetails && (
                              <div className="mb-4">
                                <Label>Description</Label>
                                    ...
                                    <div key={negIndex} className="p-3 bg-gray-50 rounded-lg">
                                      <div className="flex justify-between items-start mb-2">
                                        <span className="font-medium text-sm">
                                          {negotiation.fromClient ? 'You' : 'Provider'}
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500">Service providers haven't submitted any quotations for this request.</p>
                  </div>
                )}
              </TabsContent>
              ...
              <TabsContent value="cost-breakdown">
                {selectedRequest.costDetails ? (
                    ...
                    <CardContent className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {selectedRequest.costDetails.services?.map((service, index) => (
                      ...
                      {/* Additional Costs */}
                      {selectedRequest.costDetails.additional?.length > 0 && (
                        <div>
                          <h4 className="font-semibold mb-3">Additional Costs</h4>
                          <div className="space-y-2">
                            {selectedRequest.costDetails.additional.map((cost, index) => (
                      ...
                      <div className="border-t pt-4 space-y-2">
                        <div className="flex justify-between">
                          <span>Subtotal:</span>
                          <span>{formatCurrency(selectedRequest.costDetails.totals.baseCost)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Tax:</span>
                          <span>{formatCurrency(selectedRequest.costDetails.totals.tax)}</span>
                        </div>
                        {selectedRequest.costDetails.totals.additional > 0 && (
                          <div className="flex justify-between">
                            <span>Additional:</span>
                            <span>{formatCurrency(selectedRequest.costDetails.totals.additional)}</span>
                          </div>
                        )}
                        <div className="flex justify-between text-lg font-bold border-t pt-2">
                          <span>Total:</span>
                          <span className="text-green-600">
                            {formatCurrency(selectedRequest.costDetails.totals.grandTotal)}
                          </span>
                        </div>
                      </div>
                    </CardContent>
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">No cost details available</h3>
                    <p className="text-gray-500">Cost breakdown will appear when the request is processed.</p>
                  </div>
                )}
              </TabsContent>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        This message will be added to the communication history for this request.
                      </p>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Label htmlFor="isInternal" className="text-sm">
                        Internal note (visible only to you and service providers)
                      </Label>
                    </div>

                    <Button 
                      onClick={handleAddNote} 
                      disabled={!noteForm.content.trim() || actionLoading}
                    ...
                    {selectedRequest.internalNotes && selectedRequest.internalNotes.length > 0 ? (
                      <div className="space-y-4">
                        {selectedRequest.internalNotes
                          ...
                          .map((note, index) => {
                            const noteTypeConfig = NOTE_TYPES.find(type => type.value === note.noteType) || NOTE_TYPES[0];
                              ...
                              <div key={index} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                                <div className="flex justify-between items-start mb-3">
                                  ...
                                  <div className="text-right">
                                    ...
                                    <p className="text-xs text-gray-400">
                                      {new Date(note.addedAt).toLocaleTimeString('en-US', {
                                        hour: '2-digit',
                                        minute: '2-digit'
                                      })}
                                    </p>
                                ...
                                <div className="flex items-center justify-between text-sm">
                                  <div className="flex items-center space-x-2 text-gray-500">
                                    ...
                                    <span>By {note.addedBy?.name || 'You'}</span>
                                  </div>
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-500 mb-4">
                          Start the conversation by adding your first note above.
                        </p>
                      </div>
                    )}
                  </CardContent>
        ...
        <DialogContent className="max-w-2xl">
          ...
          {selectedQuotation && (
            <div className="space-y-6">
              ...
              <Card className="bg-blue-50">
                <CardContent className="p-4">
                  <div className="flex justify-between items-center">                    <div>
                      <p className="text-sm text-gray-600">Current Quotation</p>
              ...
              <div className="space-y-4">
                <div>
                  <Label htmlFor="proposedAmount">Your Counter Offer (Optional)</Label>
                  <Input
                    id="proposedAmount"
                    type="number"
                ...
                <div>
                  <Label htmlFor="counterOffer">Additional Terms (Optional)</Label>
                  <Textarea
                    id="counterOffer"
